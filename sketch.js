...
var boy,boyImg,boyjump,boyslide;
...
function setup() {
	...
	boy.addAnimation("slide",boyslide);
	boy.scale=1.1;
...
function draw() {
  ...
  if(gameState === PLAY){
   ...
   boy.changeAnimation("walking",boyImg);
   }
  ...
  if(keyDown("RIGHT_ARROW") && boy.y >= 210 && boy.x < displayWidth-displayWidth-20 ){
	//boy.changeAnimation("walking",boyImg);
	boy.velocityY = 0 ;
	boy.velocityX = 5;
	console.log(boy);
  ...
  if(keyDown("DOWN_ARROW") && boy.y >= 510){
	boy.changeAnimation("slide",boyslide);  
	boy.velocityY = 5 ;
	boy.velocityX = 1.2;
	console.log(boy);
  }
